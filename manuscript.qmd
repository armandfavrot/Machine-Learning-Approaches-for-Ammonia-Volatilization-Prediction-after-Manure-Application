...
```{r}
load (file = "scripts/data_alfam2.Rdata")
```
...
alfam2_predictions =  alfam2 (
        ...
        # Important to use alfam2pars01 parameters, which correspond to the parameters used in the paper
        # New parameters are available, but leads to ohter results (less good results)
        pars = pars,
...
```{r}
load (file = "scripts/data_random_forest.Rdata")
```
...
random_forest_model = randomForest (
        ...
        mtry = 22, nodesize = 3, replace = FALSE, sample_frac = 0.8
...
```{r}
load (file = "scripts/data_xgboost.Rdata")
```
...
xgboost_model = xgboost (  
    ...
    max.depth = 4, nrounds = 150, eta = 0.3, min_child_weight = 0.3, subsample = 1,
...
```{r}
load (file = "scripts/data_lasso.Rdata")
```
...
```{r}
plot = predictions_of_all_methods %>% 
...
```{r}
unified_model_rf <- randomForest.unify(random_forest_model, data_random_forest_calibration %>% select (- e.cum))

treeshap_rf <- treeshap(unified_model_rf, data_random_forest_calibration %>% select (- e.cum))
...
treeshap_xgboost <- treeshap(unified_model_xgboost, df_tmp)
...
```{r}
load (file = "scripts/data_scenarios.Rdata")
...
```{r}
load (file = "scripts/data_scenarios_random_forest.Rdata")
...
```{r}
load (file = "scripts/data_scenarios_xgboost.Rdata")
...
data_scenarios = data_scenarios %>% 
    ...
    mutate (e.cum_hat_xgboost = ifelse (e.cum_hat_xgboost < 0, 0, e.cum_hat_xgboost)) %>%
...
```{r}
load (file = "scripts/data_scenarios_lasso.Rdata")
...
```{r}
load (file = "scripts/data_scenarios_alfam2.Rdata")
...
plot_shap_rf_2 = plot_shap_rf +
    annotate (geom = "label", label = "A", size = 22, fontface = "bold", x = 58, y = 1.5, fill = "black", color = "white") +
    xlim (c (- 20, 58)) +
    theme (legend.position = "none", 
...
plot_shap_xgboost_2 = plot_shap_xgboost +
    annotate (geom = "label", label = "B", size = 22, fontface = "bold", x = 58, y = 1.5, fill = "black", color = "white") +
    xlim (c (- 20, 58)) + 
    theme (legend.position = "none", 
          plot.margin = unit (c (0, 0, 1, 0), "cm"),
