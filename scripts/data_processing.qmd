...
data_alfam2 = data_interval_level %>%
    ...
    left_join (
        ...
        data_plot_level %>%
            filter (pmid %in% pmids$pmid) %>%
            select (pmid, tan.app, app.method, app.rate, man.dm, man.ph, man.source, incorp, time.incorp) %>%
    ...
    # we keep only trials that last more than 24h, to be in the same settings as for the machine learning models
    filter (pmid %in% pmid [max (ct) > 24.5], .by = pmid) %>%

    rename (time = ct) %>%

    {.}

save (data_alfam2, file = "data_alfam2.Rdata")
```
...
#     
#     time = seq (0, (time_max - 0.1), by = 0.1)
#     
#     group = c(
#         sapply (c (1 : 6), function (j) rep (j, 40)) %>% as.vector,
#         rep (7, (length (time) - 240))
#     )
#     
...
# 
#     data_tmp = cbind (
#         data_tmp[nrow (data_tmp), ] %>% select (dataset, e.cum, e.rel, pmid, time, tan.app, app.mthd, app.rate, man.dm, man.ph, man.source, incorp, t.incorp),
#         df_temp,
#         df_wind,
#         df_rain
#     )
...
# save (data_ml, file = "data_ml.Rdata")

load (file = "data_ml.Rdata")
```
...
save (data_random_forest, file = "data_random_forest.Rdata")
```
...
save (data_xgboost, file = "data_xgboost.Rdata")
```
...
save (data_lasso, file = "data_lasso.Rdata")
```
...
data_ml %>%
    ...
    select (rain_1, rain_2, rain_3, rain_4, rain_5, rain_6, rain_7) %>%

    apply (2, quantile, probs = seq (0.01, 1, by = 0.01)) %>%
    as.data.frame %>%
    mutate (quantile = row_number () / 100) %>%
    pivot_longer (cols = - c (quantile)) %>%

    filter (quantile > 0.75) %>%
    ggplot () +
        geom_line (aes (x = quantile, y = value, color = name)) +
...
```{r}
grid_climatic_variable = quartiles_climatic_variable [, c(1 : 7)] %>% 
    cross_join (quartiles_climatic_variable [, c(8 : 14)]) %>% 
    cross_join (quartiles_climatic_variable [, c(15 : 21)]) %>% 

    mutate (group_temp = ifelse (temp_1 == quartiles_climatic_variable[1, 1], "q1", "q2")) %>%
    mutate (group_wind = ifelse (wind_1 == quartiles_climatic_variable[1, 8], "q1", "q2")) %>%
    mutate (group_rain = ifelse (rain_2 == quartiles_climatic_variable[1, 16], "q1", "q2"))
```
...
save (data_scenarios, file = "data_scenarios.Rdata")
```
...
save (data_scenarios_random_forest, file = "data_scenarios_random_forest.Rdata")
```
...
save (data_scenarios_xgboost, file = "data_scenarios_xgboost.Rdata")
```
...
save (data_scenarios_lasso, file = "data_scenarios_lasso.Rdata")
```
...
data_scenarios_alfam2 = data_scenarios %>% 
    ...
    pivot_longer (cols = c (10 : 30), names_to = c (".value", "time"), names_pattern = "(.+)_(.+)") %>%

    select (- group_temp, - group_wind, - group_rain) %>%

    rename (air.temp = temp, wind.2m = wind, rain.rate = rain) %>%

    mutate (time = as.numeric (recode (time, "1" = "4", "2" = "8", "3" = "12", "4" = "16", "5" = "20", "6" = "24", "7" = "72"))) %>%
    mutate (dt = ifelse (time == 72, (72 - 24), 4), .after = time) %>%

    mutate (rain.cum = cumsum (rain.rate), .by = pmid)

save (data_scenarios_alfam2, file = "data_scenarios_alfam2.Rdata")
```
